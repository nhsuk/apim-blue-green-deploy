...
const supportedApiVersions = require('./SupportedApiVersions');

module.exports = async function azureSearchRequest(searchUrl, options = {}) {
  const method = options.method || 'get';
  const body = options.body || '{}';
  const searchApiVersion = options.searchApiVersion || process.env['search-api-version'];

  if (!supportedApiVersions.Search.includes(searchApiVersion)) {
