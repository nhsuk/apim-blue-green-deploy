...
module.exports = df.orchestrator(function* orchestratorFunctionGenerator(context) {
  ...
  const indexingDetails = yield context.df.callActivity('GetIndexingDetails');
  const indexerUrl = yield context.df.callActivity('ReIndex', indexingDetails);
  const polling = { interval: 60, units: 'seconds' };
  const expiryTime = moment().add(polling.interval * 15, polling.units);

  const idleIndexUrl = indexingDetails.idle.url;
  while (moment.utc(context.df.currentUtcDateTime).isBefore(expiryTime)) {
    ...
    const indexerStatus = yield context.df.callActivity('GetIndexerStatus', indexerUrl);
